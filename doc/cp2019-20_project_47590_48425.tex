...
After inspecting the profiler results in the 16 thread CPU, we noticed that the added weight resulted in a very significant reduction of the percentage of time spent in creating the threads. So we decided to run all tests with the added weight on the cluster.
...
After adding the weight, with 10.000.000 iterations on the 16-thread CPU we could check on the profiler time spent in management drop from ~95\% to ~65\%. This means the management cost is still very significant even with the added weight.
...
\subsubsection{Testing}
The scan did not show improvements from switching from 1 to 2 threads in every number iterations, with the added management costs being higher than the gains up to 1.000.000 iterations (the maximum we've tested). But from upward of 2 threads, the gains were linear up to 32 threads, with very small improvement from 32 to 64 threads.
